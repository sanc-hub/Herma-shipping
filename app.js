...
function renderMain(){
  ...
  if(dz && fp){
    // Click to open file dialog after Drive auth is available.
    dz.addEventListener('click', async () => {
      ...
      try {
        ...
        console.error('Google auth preparation failed:', err);
        if(err && err.message === 'DOMAIN_NOT_CONFIGURED'){
          const origin = window.location.origin || 'unknown origin';
          toast('Domain not configured in Google Cloud. Authorize this origin: '+origin,'error');
        } else if(err && /popup|blocked|timed out/i.test(err.message)){
          toast('Google auth popup blocked. Allow popups in browser settings.','warn');
        } else {
          toast('Google Drive authorization is required before upload.','warn');
        }
        return;
      }
      fp.click();
